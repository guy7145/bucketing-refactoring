...
def record_bfs(graph, src_nodes: set):
    recorded_nodes = set()
...
def calculate_slide(control_edges, leaf):
...
def slides_first_search(sdg, target_nodes, search_direction):
...
def union_slides(leaves, slides):
    union = set()
    for l in leaves:
...
def slide_based_bucketing(V, control_edges, data_edges, M):
    ...
    return union_slides(marked, slides), \
